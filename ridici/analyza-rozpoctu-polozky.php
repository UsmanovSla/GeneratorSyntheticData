...
?>
<h2>Analýza rozpoètu dle položek: Seznam staveb</h2>
...
<?php
		...
		echo "<a href='analyza-rozpoctu-polozky?ids=".$id."' title='Analýza rozpoètu dle kalkulaèního vzorce'><img src='./skin/button/16/view.png' alt='Analýza rozpoètu dle kalkulaèního vzorce' width='16px' height='16px' border='0'></a>";
		echo "</td>";
		echo "</tr>";
	}
...
?>
<h2>Analýza rozpoètu dle položek: Stavba: <?php echo $nazevst;?>. Cena díla: <?php echo str_replace(" ","&nbsp;",number_format($cena,0,'',' '));?>,-Kè</h2>
...
&nbsp;&nbsp;
<?php } ?>
<a title="Zpìt na hlavní stránku" href="analyza-rozpoctu-polozky"><img src='./skin/button/32/sort.png' alt='Zpìt na hlavní stránku' width='32px' height='32px' border='0'></a>
<?php
...
<?php
	...
	while ($clanky3 = mysql_fetch_array($getclanek3)){
		...
		$cenasekce=0;
		if ($urovenstr==2) $getclanek = mysql_query("SELECT * FROM `Rozpocet` where kod_wbs='".$kod_wbsstr."' and stavba='".$st."'",$connect2);
		else $getclanek = mysql_query("SELECT * FROM `Rozpocet` where kod_wbs like '".substr($kod_wbsstr,0,3)."%' and stavba='".$st."'",$connect2);
		while ($clanky = mysql_fetch_array($getclanek)){
		...
		if ($koefskutec<($min-0.1)) $dops="<td class='row".$row."' align='right'><img src='skin/button/wa.png' width='24px' height='24px'></td>";
		else if ($koefskutec>($max+0.1)) $dops="<td class='row".$row."' align='right'><img src='skin/button/wa2.png' width='24px' height='24px'></td>";
		...
		echo "<td class='row".$row."' align='center'><span class='name2'><em>".$min."%-".$max."%</em></span></td>";
